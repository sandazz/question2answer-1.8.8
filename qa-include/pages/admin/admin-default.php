...
$reqPart = qa_request_part(1);
$adminsection = strtolower(isset($reqPart) ? $reqPart : '');
	...
	if (empty($optionname)) {
		...
		switch ($optionname) { // special treatment for certain options
			...
			case 'site_theme_mobile':
				...
				if (strlen(isset($metadata['version']) ? $metadata['version'] : ''))
					$namehtml = 'v' . qa_html($metadata['version']);
				else
					$namehtml = '';

				if (strlen(isset($metadata['uri']) ? $metadata['uri'] : '')) {
					if (!strlen($namehtml))
				...
				$authorhtml = '';
				if (strlen(isset($metadata['author']) ? $metadata['author'] : '')) {
					$authorhtml = qa_html($metadata['author']);

					if (strlen(isset($metadata['author_uri']) ? $metadata['author_uri'] : ''))
				...
				$updatehtml = '';
				if (strlen(isset($metadata['version']) ? $metadata['version'] : '') && strlen(isset($metadata['update_uri']) ? $metadata['update_uri'] : '')) {
