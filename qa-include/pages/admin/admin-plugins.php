...
if (!empty($fileSystemPlugins)) {
	...
	foreach ($sortedPluginFiles as $pluginDirectory => $metadata) {
		...
		$metaver = strlen(isset($metadata['version']) ? $metadata['version'] : '');
		if ($metaver)
		...
		if ($metaver && strlen(isset($metadata['update_uri']) ? $metadata['update_uri'] : '')) {
		...
		elseif (qa_php_version_below(isset($metadata['min_php']) ? $metadata['min_php'] : ''))
			$pluginhtml = '<s style="color:#999">'.$pluginhtml.'</s><br><span style="color:#f00">'.
