...
function qa_page_q_edit_q_submit($question, $answers, $commentsfollows, $closepost, &$in, &$errors)
{
	...
	if (!qa_check_form_security_code('edit-' . $question['postid'], qa_post_text('code')))
		...
		if (array_key_exists('categoryid', $in) && strcmp(isset($in['categoryid']) ? $in['categoryid'] : '', isset($question['categoryid']) ? $question['categoryid'] : '')) {
		...
		if (empty($errors)) {
			...
			if (qa_using_categories() && strcmp(isset($in['categoryid']) ? $in['categoryid'] : '', isset($question['categoryid']) ? $question['categoryid'] : '')) {
