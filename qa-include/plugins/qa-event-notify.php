	...
	public function process_event($event, $userid, $handle, $cookieid, $params)
	{
		...
		switch ($event) {
			case 'q_post':
				...
				$sendhandle = isset($handle) ? $handle : (strlen((string)$params['name']) ? $params['name'] : qa_lang('main/anonymous'));
			...
			case 'a_post':
				...
				if (isset($question['notify']) && !qa_post_is_by_user($question, $userid, $cookieid))
					qa_send_notification($question['userid'], $question['notify'], @$question['handle'], qa_lang('emails/q_answered_subject'), qa_lang('emails/q_answered_body'), array(
						'^a_handle' => isset($handle) ? $handle : (strlen((string)$params['name']) ? $params['name'] : qa_lang('main/anonymous')),
					...
					qa_send_notification(null, qa_opt('feedback_email'), null,
						...
						array(
							'^p_handle' => isset($handle) ? $handle : (strlen($params['name']) ? $params['name'] :
									(strlen(isset($oldquestion['name']) ? $oldquestion['name'] : '') ? $oldquestion['name'] : qa_lang('main/anonymous'))),
							'^p_context' => trim((isset($params['title']) ? $params['title'] : '') . "\n\n" . $params['text']), // don't censor for admin
							'^url' => qa_q_path($params['postid'], $params['title'], true),
			...
			case 'a_requeue':
				if (qa_opt('moderate_notify_admin')) {
					qa_send_notification(null, qa_opt('feedback_email'), null,
						...
						array(
							'^p_handle' => isset($handle) ? $handle : (strlen($params['name']) ? $params['name'] :
								(strlen(isset($oldanswer['name']) ? $oldanswer['name'] : '') ? $oldanswer['name'] : qa_lang('main/anonymous'))),
			...
			case 'c_requeue':
				if (qa_opt('moderate_notify_admin')) {
					qa_send_notification(null, qa_opt('feedback_email'), null,
						...
						array(
							'^p_handle' => isset($handle) ? $handle : (strlen($params['name']) ? $params['name'] :
								(strlen(isset($oldcomment['name']) ? $oldcomment['name'] : '') ? $oldcomment['name'] : // could also be after answer converted to comment
									(strlen(isset($oldanswer['name']) ? $oldanswer['name'] : '') ? $oldanswer['name'] : qa_lang('main/anonymous')))),
							'^p_context' => $params['text'], // don't censor for admin
