...
function qa_eval_from_file($eval, $filename)
{
	...
	if (isset($php_errormsg)) {
		$showErr = ini_get('display_errors');
		switch (strtolower(isset($showErr) ? $showErr : '')) {
			case 'on':
			case '1':
			case 'yes':
			case 'true':
...
function qa_fatal_error($message)
{
	...
	foreach ($backtrace as $trace) {
		$color = strpos(isset($trace['file']) ? $trace['file'] : '', '/qa-plugin/') !== false ? 'red' : '#999';
		echo sprintf(
			'<code style="color: %s">%s() in %s:%s</code><br>',
			$color, qa_html(isset($trace['function']) ? $trace['function'] : ''), basename(isset($trace['file']) ? $trace['file'] : ''), isset($trace['line']) ? $trace['line'] : ''
		);
	}
...
function qa_load_module($type, $name)
{
	...
	if (is_array($module)) {
		...
		if (strlen(isset($module['include']) ? $module['include'] : ''))
			require_once $module['directory'] . $module['include'];

		if (strlen(isset($module['class']) ? $module['class'] : '')) {
...
function qa_sanitize_html_hook_tag($element, $attributes = null)
{
	...
	if ($element == 'param' && trim(strtolower(isset($attributes['name']) ? $attributes['name'] : '')) == 'allowscriptaccess')
...
function qa_is_mobile_probably()
{
	...
	$loweragent = strtolower(isset($_SERVER['HTTP_USER_AGENT']) ? $_SERVER['HTTP_USER_AGENT'] : '');
	...
	return qa_string_matches_one(strtolower(isset($_SERVER['HTTP_ACCEPT']) ? $_SERVER['HTTP_ACCEPT'] : ''), array(
