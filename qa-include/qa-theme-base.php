...
class qa_html_theme_base
{
	...
	public function output_split($parts, $class, $outertag = 'span', $innertag = 'span', $extraclass = null)
	{
		...
		$this->output(
			...
			(strlen(isset($parts['prefix']) ? $parts['prefix'] : '') ? ('<' . $innertag . ' class="' . $class . '-pad">' . $parts['prefix'] . '</' . $innertag . '>') : '') .
			(strlen(isset($parts['data']) ? $parts['data'] : '') ? ('<' . $innertag . ' class="' . $class . '-data">' . $parts['data'] . '</' . $innertag . '>') : '') .
			(strlen(isset($parts['suffix']) ? $parts['suffix'] : '') ? ('<' . $innertag . ' class="' . $class . '-pad">' . $parts['suffix'] . '</' . $innertag . '>') : ''),
			'</' . $outertag . '>'
		);
	}
	...
	public function head_metas()
	{
		...
		if (strlen(isset($this->content['keywords']) ? $this->content['keywords'] : '')) {
	...
	public function nav_link($navlink, $class)
	{
		if (isset($navlink['url'])) {
			$this->output(
				...
				'"' . (strlen(isset($navlink['popup']) ? $navlink['popup'] : '') ? (' title="' . $navlink['popup'] . '"') : '') .
		...
		if (strlen(isset($navlink['note']) ? $navlink['note'] : ''))
	...
	public function attribution()
	{
		...
		$this->output(
			...
			'Powered by <a href="http://www.question2answer.org/">JAAN Network</a>',
			'</div>'
		);
	}
	...
	{
		if (strlen(isset($part['title']) ? $part['title'] : '') || strlen(isset($part['title_tags']) ? $part['title_tags'] : ''))
			$this->output('<h2' . rtrim(' ' . (isset($part['title_tags']) ? $part['title_tags'] : '')) . '>' . (isset($part['title']) ? $part['title'] : '') . '</h2>');
	}
	...
	public function form_button_data($button, $key, $style)
	{
		...
		$this->output('<input' . rtrim(' ' . (isset($button['tags']) ? $button['tags'] : '')) . ' value="' . (isset($button['label']) ? $button['label'] : '') . '" title="' . (isset($button['popup']) ? $button['popup'] : '') . '" type="submit"' .
	...
	{
		$this->output('<div class="qa-q-list-item' . rtrim(' ' . (isset($q_item['classes']) ? $q_item['classes'] : '')) . '" ' . (isset($q_item['tags']) ? $q_item['tags'] : '') . '>');
	...
	public function post_meta($post, $class, $prefix = null, $separator = '<br/>')
	{
		...
		$order = explode('^', isset($post['meta_order']) ? $post['meta_order'] : '');
	...
	public function post_meta_who($post, $class)
	{
		if (isset($post['who'])) {
			...
			if (strlen(isset($post['who']['prefix']) ? $post['who']['prefix'] : ''))
			...
			if (strlen(isset($post['who']['suffix']) ? $post['who']['suffix'] : ''))
	...
	{
		if (!empty($q_view)) {
			$this->output('<div class="qa-q-view' . (@$q_view['hidden'] ? ' qa-q-view-hidden' : '') . rtrim(' ' . (isset($q_view['classes']) ? $q_view['classes'] : '')) . '"' . rtrim(' ' . (isset($q_view['tags']) ? $q_view['tags'] : '')) . '>');
